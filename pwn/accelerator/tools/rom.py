...
parser.add_argument(
    "--len", help="maximum number of instructions", type=int, default=4096
)
...
run(
    f"{zig} build-exe {str(file)} -mcpu=baseline_rv32-m-a-d-f-c-zicsr -target riscv32-freestanding -femit-bin={str(romfile)} {flags}",
    check=True,
    shell=True,
)
